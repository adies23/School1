...
export default function AddStudyField  ({ open, close })  {
    ...
    const addData = async (data) => {
        console.log('Start add Data to StudyFields');
        const res = await axios.post('http://localhost:5000/api/StudyFields',data)
        .then(({ data }) => {
        ...
        setIsLoading(false);
        close(); 
        console.log('End add Data to StudyField');
    }    
    ...
    <div className='container'>
        <Dialog PaperProps={{ sx: { borderRadius: "4%" } }} open={open} fullWidth maxWidth='sm'>
            <form onSubmit={onSubmit}>
                <DialogTitle>Add StudyFields</DialogTitle>
                <DialogContent>
